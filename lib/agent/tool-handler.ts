...
import { executeWriteStatus, executeAddNote, executeSearchDatabase, executeShowExcelPreview, executeGetContactSnapshot, executeGetWaitlistSummary } from '../tools/executors'
import type { WriteStatusParams, AddNoteParams, SearchDatabaseParams, ShowExcelPreviewParams, GetContactSnapshotParams, GetWaitlistSummaryParams } from '../tools/definitions'
...
export async function handleToolCall(toolName: string, args: any): Promise<ToolResult> {
  ...
  try {
    switch (toolName) {
      ...
      case 'getWaitlistSummary':
        return await executeGetWaitlistSummary(args as GetWaitlistSummaryParams)

      default:
        console.error(`❌ Unknown tool: ${toolName}`)
        return {
          success: false,
          message: `Unknown tool: ${toolName}. Available tools: writeStatusToContact, addNoteToContact, searchDatabase, showExcelPreview, getContactSnapshot, getWaitlistSummary`
        }
    }
  } catch (error) {
